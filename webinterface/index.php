...
<tr><td>Noise Reduction (0 for off, ex 0.19):</td><td> <input size=4 type=text value="<?php if (isset($_POST['noisered'])) { echo $_POST['noisered']; } else { echo "0"; }?>" name="noisered"></td></tr>
<tr><td>Noise Reduction Profile:</td><td><select id="noiseprof" name="noiseprof">
<?php
foreach (glob("/var/www/html/gb/noiseprof/*.prof") as $filename) {
...
?>
<option value="createnew" <?php if (isset($_POST['noiseprof']) && $_POST['noiseprof'] == "createnew") echo "selected" ?> >Create new</option>
</select></td></tr>
<tr><td>Highpass Filter (0 for off):</td><td> <input size=4 type=text value="<?php if (isset($_POST['highpass'])) { echo $_POST['highpass']; } else { echo "0"; }?>" name="highpass"></td></tr>
<tr><td>Lowpass Filter (0 for off):</td><td> <input size=4 type=text value="<?php if (isset($_POST['lowpass'])) { echo $_POST['lowpass']; } else { echo "0"; }?>" name="lowpass"></td></tr>
<tr><td>Sinc Filter (0 for off):</td><td> <input size=10 type=text value="<?php if (isset($_POST['sinc'])) { echo $_POST['sinc']; } else { echo "0"; }?>" name="sinc"></td></tr>
<tr><td>Compand Filter (0 for off):</td><td> <input size=20 type=text value="<?php if (isset($_POST['compand'])) { echo $_POST['compand']; } else { echo "0"; }?>" name="compand"></td></tr>
<tr><td>Buffer (for sox/play, do not use with noise reduction!):</td><td> <input size=4 type=text value="<?php if (isset($_POST['buffer'])) { echo $_POST['buffer']; } else { echo "0"; }?>" name="buffer"></td></tr>
...
<tr><td colspan="2" align="center"><input type="submit" value="Delete Dynamic Noise Profiles" name="DeleteNoiseProf" class="blue-submit"/></td></tr>
...
<a href="/gb/recordings">Recordings</a> | <a href="/gb/index-ffmpeg.php">FFMPEG Web Interface</a>
<br><br>
</center>
<?php
    if (isset($_POST['Start'])) {
		...
		$cmd="sudo -u pigb /home/pigb/startgb.sh ".$_POST['hopmode']." ".$_POST['hopdelay']." ".$_POST['minutes']." ".$dorecord." ".$_POST['resamplerate']." ".$_POST['gain']." ".$_POST['frequency']." ".$_POST['samplerate']." ".$_POST['noisered']." ".$_POST['volume']." ".$_POST['modulation']." ".$_POST['engine']." ".$_POST['buffer']." ".$dostream." '".$_POST['noiseprof']."' ".$_POST['highpass']." ".$_POST['lowpass']." '".$_POST['sinc']."' '".$_POST['compand']."'&";
