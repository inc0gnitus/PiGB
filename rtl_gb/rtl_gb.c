 ...
 *
 * Mods for rtl_gb made in 2022 by Incognitus <meincognitus@gmail.com>
 *
...
#define FREQUENCIES_LIMIT		100000 //RTL_GB
...
struct dongle_state
{
	...
	struct demod_state *demod_target;
        ...
        int      mode;    //RTL_GB
        int     step;     //RTL_GB
        uint32_t freq_start; //RTL_GB
        uint32_t freq_stop;  //RTL_GB
};
...
{
	fprintf(stderr,
                "rtl_gb, Virtual ghostbox based on rtl_fm\n\n"
		"Use:\trtl_gb -f freq [-options] [filename]\n"
		"\t-c Hop delay in milliseconds (default: 1, 1 second equals 1000 milliseconds, -1 for random delay)\n"
		"\t-m Hop mode (default: 0, 0 = random, 1 = sequential forward, 2 = sequential backwards)\n"
...
static void *controller_thread_fn(void *arg)
{
	...
	int i;
        float count=0; //RTL_GB
        float newdelay=0; //RTL_GB
        int countmax=5; //RTL_GB
        uint32_t new_freq; //RTL_GB
        uint32_t diff_freq; //RTL_GB
	...
	while (!do_exit) {
		...
		//dongle.mute = BUFFER_DUMP;
                if (dongle.mode==0) {
                       new_freq = getrand(dongle.freq_start, dongle.freq_stop);
                } else if (dongle.mode==1) {
                       new_freq = dongle.freq + dongle.step;
                       if (new_freq>dongle.freq_stop) {
                               diff_freq = new_freq - dongle.freq_stop;
                               new_freq = dongle.freq_start+diff_freq;
                       }
                 } else {
                       new_freq = dongle.freq - dongle.step;
                       if (new_freq<dongle.freq_start) {
                               diff_freq = dongle.freq_start - new_freq;
                               new_freq = dongle.freq_stop-diff_freq;
                       }
                 }
                 if (dongle.count==-1)
                        newdelay = getrand(0,600);
                 else
                        newdelay = dongle.count;

                 //fprintf(stderr, "DEBUG: Random: %u Hz, Start: %u Hz, End: %u Hz, Step: %u, Output now at %u Hz. Delay: %f\n", new_freq, dongle.freq_start, dongle.freq_stop, dongle.step, dongle.freq, newdelay);
...
void frequency_range(struct controller_state *s, char *arg)
{
	...
	for(i=(int)atofs(start); i<=(int)atofs(stop); i+=(int)atofs(step))
	{
        ...
        dongle.freq_start=(int)atofs(start); //RTL_GB
        dongle.freq_stop=(int)atofs(stop); //RTL_GB
        dongle.step=(int)atofs(step); //RTL_GB
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "c:d:f:g:s:b:l:o:t:r:p:E:F:A:m:M:hT")) != -1) { //RTL_GB
		switch (opt) {
                case 'c': //RTL_GB
                        dongle.count = (float)atof(optarg);
                        break;
		case 'd':
